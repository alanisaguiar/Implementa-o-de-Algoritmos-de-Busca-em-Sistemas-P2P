...
class P2PNetwork:
    ...
    def _animate_flooding(self, start_node, resource_name, ttl):
        ...
        found_id = None  # guarda o nó onde o recurso foi achado (sem parar a busca)

        while queue:
            current_node, current_ttl = queue.popleft()

            # ATUALIZA O GRÁFICO (Nó atual laranja; nó achado fica verde)
            self._update_plot(current_id=current_node.id, visited=visited, found_id=found_id)

            # Verifica sucesso, mas NÃO para: a inundação segue por toda a rede
            if resource_name in current_node.resources and found_id is None:
                print(f"✅ Visual: Recurso encontrado em {current_node.id}!")
                found_id = current_node.id
        ...
        # Quadro final: mostra toda a rede inundada com o nó do recurso em verde
        self._update_plot(visited=visited, found_id=found_id)
    ...
    def _algo_random_walk(self, start_node, resource_name, ttl, is_informed=False):
        ...
        visited = set([start_node.id])
        nodes_involved = set([start_node.id])
        # pilha = caminho atual percorrido (permite voltar = backtracking)
        pilha = [start_node]
        found = False
        target_id = None

        # O nó inicial já tem o recurso?
        if resource_name in start_node.resources:
            return self._rw_result(True, start_node.id, msgs_count, nodes_involved)

        while pilha and ttl > 0:
            current_node = pilha[-1]

            # BUSCA INFORMADA: o nó atual sabe (pelo cache) onde está o recurso?
            if is_informed and resource_name in current_node.cache:
                ...
                return self._rw_result(True, destino, msgs_count, nodes_involved)

            # Vizinhos ainda NÃO visitados (não repete nós)
            nao_visitados = [n for n in current_node.neighbors if n.id not in visited]

            if nao_visitados:
                # Avança para um vizinho novo escolhido aleatoriamente
                proximo = random.choice(nao_visitados)
                msgs_count += 1
                ttl -= 1
                visited.add(proximo.id)
                nodes_involved.add(proximo.id)
                pilha.append(proximo)

                if resource_name in proximo.resources:
                    found = True
                    target_id = proximo.id
                    print(f"✅ Recurso encontrado no nó {proximo.id}!")
                    break
            else:
                # BACKTRACKING: nó sem vizinhos novos -> volta para o anterior
                pilha.pop()
                if pilha:
                    msgs_count += 1  # mensagem de retorno avisando "não achei por aqui"
                    print(f"↩️  Backtracking: voltando de {current_node.id}.")

        if not found:
            print(f"❌ Busca encerrada sem encontrar '{resource_name}'.")

        return self._rw_result(found, target_id, msgs_count, nodes_involved)

    def _rw_result(self, found, target_id, msgs_count, nodes_involved):
        return {"sucesso": found, "nó_destino": target_id,
                "mensagens": msgs_count, "nós_envolvidos": len(nodes_involved),
                "conjunto_nos": nodes_involved}

    def _algo_random_walk_OLD(self, start_node, resource_name, ttl, is_informed=False):
